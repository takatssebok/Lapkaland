...
$stmt = $db->prepare('SELECT postId, postHeadline, postSlug, postDescription, created_at, postTags FROM sitePosts WHERE postTags like :postTags ORDER BY postId DESC');
...
<title><?php echo htmlspecialchars($_GET['id']);?> - Lapkaland</title>
...
<div class="container-fluid">
  <div class="row">
    ...
    <div class="content">
            <p><i><h4>Címke: <?php echo htmlspecialchars($_GET['id']);?></h4></i></p>
                <hr>
                ...
                <?php
                    ...
                    /* PHP Blog Search*/
                    $search_query = 'SELECT postId, postHeadline, postSlug, postDescription, created_at, postTags FROM sitePosts WHERE postTags like :postTags ORDER BY postId DESC';
                    ...
                    $pagination_stmt->bindValue(':postTags',  '%'.$_GET['id'].'%' , PDO::PARAM_STR);
                    ...
                    $pdo_stmt->bindValue(':postTags',  '%'.$_GET['id'].'%' , PDO::PARAM_STR);
                    $pdo_stmt->execute();
                ?>
                ...
                <?php   
                        while($row = $pdo_stmt->fetch()){
                            ...
                            echo '<div>';
                                echo '<div style="margin:10px 0px"><h2><a href="../'.$row['postSlug'].'">'.$row['postHeadline'].'</a></h1></div>';
